create or replace PACKAGE BODY ORDERENTRY AS
  --
  --
  ...
  --
    ...
    procedure sleep_now(sleep_time integer) is
    begin
        $IF DBMS_DB_VERSION.VER_LE_12
            $THEN
            dbms_lock.sleep(sleep_time);
        $ELSIF DBMS_DB_VERSION.VER_LE_18
            $THEN
            dbms_lock.sleep(sleep_time);
        $ELSE
    ...
    procedure logon(cust_id customers.customer_id%type) is
      ...
      begin
        ...
        if trunc(dbms_random.value(1,4)) = 2 then
          ...
          oecommit;
          increment_selects(1);increment_inserts(1);
        end if;
        dbms_application_info.set_action(null);
    end logon;
...
--         customerArray := getCustomerDetails(cust_id);
--         increment_selects(1);
--         transaction should be readonly
...
create or replace PROCEDURE autonomousLogon(cust_id customers.customer_id%type, logon_identifier logon.logon_id%type, logon_occurred logon.logon_date%type)  is
    pragma autonomous_transaction;
begin
    insert into logon (logon_id,customer_id, logon_date)
    values
        (logon_identifier, cust_id, logon_occurred);
    -- this is an autonomous transaction and uses an explicit commit
    commit;
end autonomousLogon;
