...
// ============================================
// GC Congestion Demo API Routes
// ============================================

// Prepare demo tables and load scaled data
app.post('/api/gc-congestion/prepare', async (req, res) => {
  const config = req.body || {};
  try {
    const result = await gcCongestionEngine.prepare(oracleDb, config, io);
    res.json({ success: true, result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
// Start GC congestion workload
app.post('/api/gc-congestion/start', async (req, res) => {
  const config = req.body || {};
  try {
    await gcCongestionEngine.start(oracleDb, config, io);
    res.json({ success: true, message: 'GC congestion workload started' });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});

// Stop GC congestion workload
app.post('/api/gc-congestion/stop', async (req, res) => {
  try {
    const stats = await gcCongestionEngine.stop();
    res.json({ success: true, message: 'GC congestion workload stopped', stats });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});

// Drop generated demo tables
app.post('/api/gc-congestion/drop', async (req, res) => {
  try {
    const result = await gcCongestionEngine.dropTables(req.body || {}, oracleDb);
    res.json({ success: true, ...result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
app.get('/api/gc-congestion/status', (req, res) => {
  res.json(gcCongestionEngine.getStatus());
});

// List available GC wait events for filtering
app.get('/api/gc-congestion/wait-events', async (req, res) => {
  try {
...
const shutdown = async (signal) => {
  console.log(`Received ${signal}. Running shutdown sequence...`);
  stressEngine.stop();
  ...
  gcCongestionEngine.stop();
  metricsCollector.stop();
  await oracleDb.close();

  // Only exit when running interactively (e.g., foreground dev server)
  if (!process.env.KEEP_ALIVE || process.env.KEEP_ALIVE !== 'true') {
    process.exit(0);
  }
};
